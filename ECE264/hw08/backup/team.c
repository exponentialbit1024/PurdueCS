...
struct Team create_team(char *name, int population, struct Person *people){
	...
	if(team.head != NULL){
		...
		for(int k = 0; k < population; k++){
			...
			favAdder -> value -> favorite -> name = find_person(&team, people[k].favorite -> name) -> name;
...
struct Person *detach_person(struct Team *team, char *name){
	struct PersonNode *searchHead = team -> head;
	struct PersonNode *tempStore = NULL;
	...
	if(strcmp(searchHead -> value -> name, name) == 0){
		...
		while(searchHead != NULL && strcmp(searchHead -> value -> name, name) != 0){
			tempStore = searchHead;
			searchHead = searchHead -> next;
		}
		if(searchHead != NULL){
			tempStore -> next = searchHead -> next;
			if(strcmp(team -> tail -> value -> name, name) == 0){
				team -> tail = tempStore;
			}
			else{
				searchHead -> next -> prev = tempStore;
			}
			returnVal = searchHead -> value;
			free(searchHead);
			return returnVal;
